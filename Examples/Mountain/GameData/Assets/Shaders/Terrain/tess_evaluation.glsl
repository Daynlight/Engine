...
float terrainHeight(vec2 p){
  ...
  float center = texture(texture0, uv).r;
  float left   = texture(texture0, uv + vec2(-offset.x, 0.0)).r;
  float right  = texture(texture0, uv + vec2(offset.x, 0.0)).r;
  float top    = texture(texture0, uv + vec2(0.0, offset.y)).r;
  float bottom = texture(texture0, uv + vec2(0.0, -offset.y)).r;
